...
rep(c(1,2),c(2,4)) #rep 1 twice, and 2 four times

#Sequencing
D <- c(seq(-10,10,5),"A") #Sequence from -10 to 10 by 5 and add string A
D

seq(3,30,length=10) #Seq such that length =10
...
##### V IMPT!!!!!
colSums(is.na(mtcars))  #Column marking and count of how many NA there is in dataset
...
#table is a function for 2 set of binary vars, the big overal 0 and 1 represents the list of factors
...
diag(c(5,3,2)) #diagonal matrix of items 5,3,2, size 3X3
...
table(celg$age<60)
length(table(celg$age))  #see how many items for age
table(celg$age)["58"]  #Look at the table values for age is 58
...
#ggplot bar plot
```{r}
bar <- ggplot(cars)+geom_bar(mapping=aes(x=cyl,))+labs(x="cyl",y="Number")
bar
```
#ggplot scatter graph
```{r}
pt <- ggplot(cars,aes(x=cyl,y=mpg))+geom_point(na.rm=TRUE)+facet_grid(.~cyl)+ggtitle("cylinder vs mpg")+xlab("cyl")+ylab("mpg")  #facet lets you do multiple plots per cyl in this case
pt

pt0 <- ggplot(cars,aes(x=cyl,y=mpg))+geom_point(na.rm=TRUE)+facet_grid(cyl~.)+ggtitle("cylinder vs mpg")+xlab("cyl")+ylab("mpg")  #facet lets you do multiple plots per cyl in this case
pt0  #vert graphs

pt1 <- ggplot(cars,aes(x=cyl,y=mpg))+geom_point(na.rm=TRUE)+ggtitle("cylinder vs mpg")+xlab("cyl")+ylab("mpg")  #No facet
pt1

pt2 <- ggplot(cars,aes(x=cyl,y=mpg))+geom_point(na.rm=TRUE)+facet_wrap(~cyl,nrow=3)+ggtitle("cylinder vs mpg")+xlab("cyl")+ylab("mpg")  #facet wrap into 3 rows
pt2
```
#Geom_histogram
```{r}
ggplot(cars,aes(x=cyl))+geom_histogram(binwidth = 1)  #Adjust binwidth

ggplot(cars, aes(x = cyl))+geom_histogram(binwidth=(1),center=4,closed=c("left"),color=c("blue"))  #center a bin on 4, outline blue

ggplot(cars, aes(x=cyl, fill=as.factor(mpg)))+geom_histogram(binwidth = 1) #Fill with color

ggplot(cars, aes(x=cyl, fill=as.factor(mpg)))+geom_histogram(binwidth = 1,position="identity",alpha=0.5) #Prevent stack and add transparency
#Overlaying doesnt work for more than 2 groups
```

#ggplot regression line
```{r}
#polynomial fit
pt <- ggplot(cars,aes(x=cyl,y=mpg))+geom_point(na.rm=TRUE)+geom_smooth(na.rm=TRUE)+ggtitle("Cyl vs MPG")+xlab("cyl")+ylab("mpg")  #Polynomial fit for regression
pt
```
```{r}
#Linear fit#####
pt <- ggplot(cars,aes(x=cyl,y=mpg))+geom_point(na.rm=TRUE)+geom_smooth(method="lm",se=F)+ggtitle("Cyl vs MPG")+xlab("cyl")+ylab("mpg") #SE=F no standard error, can put true also
pt
```
#ggplot multiple dimensions with color
```{r}
ggplot(cars,aes(x=cyl,y=mpg,color=hp))+geom_point(na.rm=TRUE)+ggtitle("cyl vs mpg vs hp")+xlab("cyl")+ylab("mpg")  #Adds color as a dimension
```
#ggplot multiple dimensions with color and size
```{r}
ggplot(cars,aes(x=cyl,y=mpg,color=hp,size=wt))+geom_point(na.rm=TRUE)+ggtitle("cyl vs mpg vs hpvs wt")+xlab("cyl")+ylab("mpg")  #Adds color and size as a dimension
```
#Adding horizontal and vertical lines in ggplot
```{r}
Add horizontal line
...
library("caTools")
set.seed(1)
split <- sample.split(framing1$TENCHD,SplitRatio = 0.65) #Split TENCHD into training and test, 65% in training, 65% true
split
training <- subset(framing1, split==T)
test <- subset(framing1, split==F)

training #Training still returns whole list, so does test
